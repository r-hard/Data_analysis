...
def z_score_normalize(data,mean=None,std=None):
    ...
    参数：
    data:带标准化的数据，可以为单个数值、列表或者numpy数组
    ...
    # 避免分母为0的情况
    if std == 0:
        raise ValueError("数据的标准差为0，不能进行数据标准化")
...
if __name__ == "__main__":
    ...
    print(f"标准化后的均值：{np.mean(normalized):.4f},标准化后的标准差：{np.std(normalized):.4f}\n")
